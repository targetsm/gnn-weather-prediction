...
class WavenetModel(nn.Module):
    def __init__(self, input_size, num_filters, kernel_size, num_residual_blocks, device):
        ...
        self.l1_conv3d = Conv3d(in_channels=num_filters, out_channels=input_size[-1], kernel_size=1,
                                padding='same').to(device)

        self.l2_conv3d = Conv3d(in_channels=input_size[-1], out_channels=input_size[-1], kernel_size=1,
                                padding='same').to(device)

        self.l3_conv3d = Conv3d(in_channels=input_size[-1], out_channels=1,
    ...
    def forward(self, l_input, y):
        ...
        l_stack_conv3d_output = self.l_stack_conv3d(l_input)
        l_skip_connections = []
        for i in range(self.num_residual_blocks):
            l_stack_conv3d_output, l_skip_connection_output = self.wave_net_res_layer_li[i](l_stack_conv3d_output)
        ...
        l2_conv3d_output = self.l2_conv3d(l1_conv3d_output)

        l3_conv3d_output = self.l3_conv3d(l2_conv3d_output)

        return l3_conv3d_output.permute(0, 4, 2, 3, 1)
